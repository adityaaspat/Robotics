...
public:
    ...
    tf_listener_(tf_buffer_)
  {
    ...
    goal_subscription_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
      "/exploration_rrt_goal", 10, std::bind(&Planner::goal_callback, this, std::placeholders::_1));
...
bool isStateValid(const ompl::base::State *state) 
{
    ...
    int x = static_cast<int>(pos->values[0]);
    int y = static_cast<int>(pos->values[1]);
    ...
    // Offsets for surrounding cells (8-connected neighborhood)
    ...
    for (int i = 0; i < 16; i += 2) 
    {
        ...
        linear_index = ny * width + nx;
        value_8_t = grid_data[linear_index];
        value = static_cast<int>(value_8_t);
        if (value >= 10) 
            return false; // Neighboring cell is not free
    }

    return true; // Current cell and its neighbors are free
}
...
void plan_path()
  {
    ...
    if (solved) {
      ...
      for (size_t i = 0; i < path.getStateCount(); i++) 
      {
        ...
        int x = static_cast<int>(state->values[0]);
        int y = static_cast<int>(state->values[1]);
        auto [odom_x, odom_y]=grid_to_world(y,x);
